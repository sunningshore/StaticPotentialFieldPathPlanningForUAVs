...
clc;
tic;
air_level=[15, 30, 45, 60, 75, 90, 105]; % Air layers selected at different heights (3D to 2D), usually same as the building thresholds 
eval(['load(''cubic_urban_map_',num2str(min(air_level)),'.mat'',''cubic_urban_map'');']);
base_cubic_urban_map=cubic_urban_map;
weight_EC_cube=[];    % Safety field 
map_img_rgb_cube=[];    % 3D city map 
cont_map_cube=[];     % Connectivity field 
...
for ind_a=1:air_level_len
    ...
    [weight_EC, map_img_rgb, cont_map, core_pt, bound_contour] = test_Contour_Map (building_height_thr);    % Calculate safety field contours at each air layer
...
%% Generate Mesh Grids %% 
grid_interval=5; 
grid_network = Grid_Generation (map_img_rgb_cube, air_level, grid_interval);    % Generate fine grids on each layer for waypoint selection 
save('grid_network.mat','grid_network');
%% Generate Trajectories %% 
traject_free=[];    % Raw result of waypoint selection 
traject_regul=[];    % Matching result of selected waypoints and fine grids 
traject_orig=[];    % Linear fitting result of selected waypoints 
figure (20)
...
for ind_c1=1:core_pt_len 
    ...
    for ind_c2=1:core_pt_len 
        ...
        % Waypoint matching with fine grids  
...
% Core Switch Points matching with fine grids  
